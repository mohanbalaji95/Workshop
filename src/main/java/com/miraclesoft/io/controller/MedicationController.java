...
import com.miraclesoft.io.ExceptionHandler.MedIdMismatchException;
import com.miraclesoft.io.ExceptionHandler.MedicationNotFoundException;
	 ...
	 public Medication_Details updateMed(@RequestBody Medication_Details med, @PathVariable Long id) {
		...
		String desc=med.getDESCRIPTION();
		 ...
		 if(med.getMEDID()!=id) {                                         //checking the employee id 
			 throw new MedIdMismatchException();
		}
		medrepo.findById(id).orElseThrow(MedicationNotFoundException::new);
		//emp.setId(eid);
		return medrepo.save(med);
	    }
